...
from sdls.gui.engine import driver, SDLs_scan, image_capture
...
class ScanningPage(WizardPage):
    ...
    def on_laser_combo_box_changed(self, event):
        ...
        SDLs_scan.set_use_left_laser(use_left)
        SDLs_scan.set_use_right_laser(use_right)

    def on_capture_texture_checkbox_changed(self, event):
        value = event.GetEventObject().GetValue()
        profile.settings['capture_texture'] = value
        SDLs_scan.set_capture_texture(value)
