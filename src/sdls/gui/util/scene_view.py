...
from sdls.util import profile, mesh_loader, model, system as sys
from sdls.gui.util import opengl_helpers, opengl_gui
...
class SceneView(opengl_gui.glGuiPanel):
        ...
        if self._object is None:
            ...
            new_view_pos = numpy.array(
                [self._object.get_position()[0],
                 self._object.get_position()[1],
                 height - self._z_offset])
            new_zoom = self._object.get_boundary_circle() * 3
    ...
    def view_left_object(self):
        ...
        self._pitch = 90
        self.center_object()
    def view_top_object(self):
        if self.isTop is True:
            self._pitch = 0
            self.isTop = False
        else:
            self._pitch = 180
    ...
    def view_front_object(self):
        if self.isFront is True:
            self._yaw = -90
            self.isFront = False
        else:
            self._yaw = 90
    ...
    def on_mouse_motion(self, e):
        if e.Dragging() and self._mouse_state is not None:
            if e.LeftIsDown() and not e.RightIsDown():
                ...
                if self._pitch < 0:
                    self._pitch = 0
            elif e.RightIsDown() and not e.LeftIsDown():
                self._mouse_state = 'drag'
                self._zoom += e.GetY() - self._mouse_y
                if self._zoom < 2:
                    self._zoom = 2
                if self._zoom > numpy.max(self._machine_size) * 5:
                    self._zoom = numpy.max(self._machine_size) * 5

            elif e.MiddleIsDown() and not e.RightIsDown() and not e.LeftIsDown():
                self._mouse_state = 'drag'
                a = math.cos(math.radians(self._yaw)) / 3.0
                b = math.sin(math.radians(self._yaw)) / 3.0
                self._view_target[0] += float(e.GetX() - self._mouse_x) * -a
    ...
    def _init_3d_view(self):
        ...
        gluPerspective(45.0, aspect, 1.0, numpy.max(self._machine_size) * 8)
    ...
    def _init_components(self):
        self._GridList = glGenLists(1);
        glNewList(self._GridList, GL_COMPILE);
        length = self.GridSize*self.GridRows/2
        glColor3f(1.0, 0, 0)
        glLineWidth(1.5)
        glBegin(GL_LINES)
        ...
        glColor3f(1, 1, 0.8)
        self.GridSize = 20
        for i in range(1, self.GridRows/2 + 1):
            glLineWidth(0.5)
            glBegin(GL_LINES)
            glVertex3d(self.GridSize*i, -length, 0)
            glVertex3d(self.GridSize*i, length, 0)
            glEnd()
        for i in range(1, self.GridRows/2 + 1):
            glBegin(GL_LINES)
        ...
        for i in range(1, self.GridRows/2 + 1):
            glBegin(GL_LINES)
            glVertex3d(-length, self.GridSize*i, 0)
            glVertex3d(length, self.GridSize*i, 0)
            glEnd()
        for i in range(1, self.GridRows/2 + 1):
            glBegin(GL_LINES)
        ...
        RCylinder = 0.2
        HCylinder = 11 * RCylinder
        RCone = 1.7 * RCylinder
        HCone = RCone * 1.4
        RSphere = 1.3 * RCylinder
        self.m_cornerAxesList = glGenLists(1)
        glNewList(self.m_cornerAxesList, GL_COMPILE)
        glPushMatrix()

        glPushMatrix()
        glColor3d(0, 0, 0)
        self._drawText( 0, 0, HCylinder + HCone + 0.2, "Z")
        glColor3d(0, 0, 0.8)
        glutSolidCylinder(RCylinder, HCylinder, 20, 2)
        glTranslated(0, 0, HCylinder)
        ...
        glColor3d(0, 0, 0)
        self._drawText(0, HCylinder + HCone + 0.2, 0, "Y")
        glRotated(90, -1, 0, 0)
        glColor3d(0, 0.8, 0)
        glutSolidCylinder(RCylinder, HCylinder, 20, 2)
        glTranslated(0, 0, HCylinder)
        ...
        glPushMatrix()
        glColor3d(0, 0, 0)
        self._drawText(HCylinder + HCone + 0.2, 0, 0, "X")
        glRotated(90, 0, 1, 0)
        glColor3d(0.8, 0, 0)
        glutSolidCylinder(RCylinder, HCylinder, 20, 2)
        glTranslated(0, 0, HCylinder)
        ...
        glPushMatrix()
        glutSolidSphere(RSphere, 30, 30)
        glPopMatrix()
        glPopMatrix()
        glEndList()

    def _corner_axes_projection(self):
        h = 4
        size = self.GetSize()
        ...
        if width < height:
            glViewport(0, 0, width/6, width/6)
        if width > height:
            glViewport(0, 0, height/6, height/6)
        glMatrixMode(GL_PROJECTION)
        glLoadIdentity()
        glOrtho(-h , h , -h, h, -2, 4)
        glMatrixMode(GL_MODELVIEW)
    ...
    def _drawText(self, X, Y, Z, text):
        glRasterPos3f(X, Y, Z)
        #cstr = text.encode('utf-8')
        for i in range(0, len(text)):
            glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_10, ord(text[i]))

    def _draw_corner_axes(self):
        glCallList(self.m_cornerAxesList)

    def _draw_grid(self):
        glCallList(self._GridList)
    ...
    def _draw_machine(self):
        ...
        # self._platform_mesh[machine_model_path]._draw_offset = numpy.array([0, 0, 8.05], numpy.float32)
