...
from sdls.util import resources, profile

from sdls.engine.driver.camera import InputOutputError

from sdls.gui.engine import driver, image_capture, laser_segmentation, calibration_data, \
    SDLs_scan, current_video, point_cloud_roi
from sdls.gui.workbench.workbench import Workbench
from sdls.gui.workbench.scanning.view_page import ViewPage
from sdls.gui.workbench.scanning.panels import ScanParameters, RotatingPlatform, \
...
class ScanningWorkbench(Workbench):
    ...
    def __init__(self, parent, toolbar_scan):
        ...
        # Elements
        self.toolbar_scan.AddStretchableSpace()
        ...
        self.toolbar_scan.AddStretchableSpace()
        self.frontView_tool = self.toolbar_scan.AddLabelTool(
            wx.NewId(), _("Front_View"),
            wx.Bitmap(resources.get_path_for_image("front_view.svg")), shortHelp=_(""))
        self.topView_tool = self.toolbar_scan.AddLabelTool(
            wx.NewId(), _("Top_View"),
            wx.Bitmap(resources.get_path_for_image("top_view.svg")), shortHelp=_(""))
        self.leftView_tool = self.toolbar_scan.AddLabelTool(
            wx.NewId(), _("Left_View"),
            wx.Bitmap(resources.get_path_for_image("left_view.svg")), shortHelp=_(""))


        self.toolbar_scan.Realize()
        self.toolbar_scan.GetParent().Layout()

        SDLs_scan.point_cloud_callback = self.point_cloud_callback
        ...
        self.toolbar_scan.GetParent().Bind(wx.EVT_TOOL, self.on_leftView_tool_clicked, self.leftView_tool)
        self.toolbar_scan.GetParent().Bind(wx.EVT_TOOL, self.on_topView_tool_clicked, self.topView_tool)
        self.toolbar_scan.GetParent().Bind(wx.EVT_TOOL, self.on_fontView_tool_clicked, self.frontView_tool)
    ...
    def setup_engine(self):
        ...
        SDLs_scan.capture_texture = profile.settings['capture_texture']
        use_laser = profile.settings['use_laser']
        SDLs_scan.set_use_left_laser(use_laser == 'Left' or use_laser == 'Both')
        SDLs_scan.set_use_right_laser(use_laser == 'Right' or use_laser == 'Both')
        SDLs_scan.motor_step = profile.settings['motor_step_scanning']
        SDLs_scan.motor_speed = profile.settings['motor_speed_scanning']
        SDLs_scan.motor_acceleration = profile.settings['motor_acceleration_scanning']
        SDLs_scan.color = struct.unpack(
            'BBB', profile.settings['point_cloud_color'].decode('hex'))
        SDLs_scan.set_scan_sleep(profile.settings['scan_sleep'])
    ...
    def on_play_tool_clicked(self, event):
        if SDLs_scan._inactive:
            ...
            if self.scene_view._object is not None:
                ...
                dlg.Destroy()
            if result:
                SDLs_scan.set_callbacks(self.before_scan,
    ...
    def on_leftView_tool_clicked(self, event):
        self.scene_view.view_left_object()

    def on_topView_tool_clicked(self, event):
        self.scene_view.view_top_object()

    def on_fontView_tool_clicked(self, event):
        self.scene_view.view_front_object()
