...
from sdls.gui.engine import driver, SDLs_scan, point_cloud_roi
from sdls.gui.util.custom_panels import ExpandablePanel, Slider, CheckBox, ComboBox, \
    Button, FloatTextBox
...
class ScanParameters(ExpandablePanel):
    ...
    def update_callbacks(self):
        self.update_callback('capture_texture', SDLs_scan.set_capture_texture)
        self.update_callback('use_laser', self.set_use_laser)

    def set_use_laser(self, value):
        SDLs_scan.set_use_left_laser(value == 'Left' or value == 'Both')
        SDLs_scan.set_use_right_laser(value == 'Right' or value == 'Both')
...
class RotatingPlatform(ExpandablePanel):
    ...
    def update_callbacks(self):
        ...
        self.update_callback('motor_step_scanning', SDLs_scan.set_motor_step)
        self.update_callback('motor_speed_scanning', SDLs_scan.set_motor_speed)
        self.update_callback('motor_acceleration_scanning', SDLs_scan.set_motor_acceleration)
...
class PointCloudColor(ExpandablePanel):
    ...
    def on_color_picker(self):
        data = wx.ColourData()
        data.SetColour(SDLs_scan.color)
        dialog = wx.ColourDialog(self, data)
        dialog.GetColourData().SetChooseFull(True)
        if dialog.ShowModal() == wx.ID_OK:
            data = dialog.GetColourData()
            color = data.GetColour().Get()
            SDLs_scan.color = color
